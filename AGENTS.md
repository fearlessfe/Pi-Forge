...
- This is a pnpm workspace. The shipped application is `apps/desktop` (`@pi-desktop/renderer`), an Electron/React reference implementation of Pi Forge. `packages/runtime-contracts` and `packages/runtime-sdk` are public stable-v1 packages, and `templates/basic-agent` is the compilable starter; other roadmap packages are not implemented architecture.
...
- Runtime/Session/Event/Hand wire types and validators belong in `@pi-forge/runtime-contracts`; transport-neutral client/host/Agent authoring APIs belong in `@pi-forge/runtime-sdk`. Desktop-only host RPC for credentials, MCP, browser access, filesystem paths, and service implementations must remain in `apps/desktop`.
