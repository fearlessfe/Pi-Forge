...
- This is a pnpm workspace. The only shipped application is `apps/desktop` (`@pi-desktop/renderer`), an Electron/React reference implementation of Pi Forge. `packages/runtime-contracts` is a real but private internal-prerelease v0 package; the other `packages/*` entries in the README remain roadmap rather than implemented architecture.
...
- Runtime/Session/Event wire types and validators belong in `@pi-forge/runtime-contracts`; desktop-only host RPC for credentials, MCP, browser access, filesystem paths, and service implementations must remain in `apps/desktop`.
