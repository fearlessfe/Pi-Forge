...
| 工具 | 支持读取、搜索、Shell、编辑、写入、用户询问和持久化只读后台子 Agent |
...
- 内置子 Agent 由专用 control Runtime 作为持久后台任务执行，支持 FIFO 队列、暂停/继续/重试/停止、重启恢复、独立历史和显式结果移交；执行仍严格限制为已授权工作区内的只读工具；
