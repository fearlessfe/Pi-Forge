    ...
    <div className="grid w-full max-w-[980px] content-start gap-3">
      ...
      {inventory && (
        <div className="flex items-center gap-base">
          <span className="min-w-[70px] rounded-sm border border-separator bg-bg-grouped px-loose py-base"><strong className="block text-body font-semibold text-label">{inventory.skills.filter((skill) => skill.enabled).length}</strong><small className="block text-caption text-label-3">{t("已启用")}</small></span>
      ...
      <section className="grid gap-base">
        {visibleSkills.map((skill) => (
          <article className={skillRowClass(skill.enabled)} key={`${skill.source}:${skill.filePath}`}>
            ...
            <span className="flex gap-tight">{!skill.modelInvocable && <em className="rounded-full border border-separator px-base py-tight text-mini not-italic text-label-3">{t("仅命令")}</em>}<em className="rounded-full border border-separator px-base py-tight text-mini not-italic text-label-3">{skill.scope}</em></span>
            <button className={switchClass(skill.enabled)} type="button" disabled={agentRunning || Boolean(busy)} aria-pressed={skill.enabled} aria-label={t(skill.enabled ? "停用 {name}" : "启用 {name}", { name: skill.name })} onClick={() => void toggle(skill.name, !skill.enabled)}><i className={switchKnobClass(skill.enabled)} /></button>
          </article>
        ))}
