...
describe("desktop runtime protocol exhaustiveness", () => {
  it("dispatches every public runtime method in the worker", () => {
    ...
    const dispatched = runtimeMethods.filter((method) => cases.has(method));
    expect(dispatched).toEqual([...runtimeMethods]);
  });
  ...
  it("exposes a typed client request for every public runtime method", () => {
    ...
    const exposed = runtimeMethods.filter((method) => requestLines.some((line) => line.includes(`"${method}"`)));
    expect(exposed).toEqual([...runtimeMethods]);
  });
});
